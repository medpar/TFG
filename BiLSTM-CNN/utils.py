...
PARENT_DIR = os.path.dirname(CURRENT_DIR) # This should be BiLSTM-CNN/
# GRANDPARENT_DIR = os.path.dirname(PARENT_DIR) # This would be vidimu-tools/
# Assuming gaitseg is a sibling of BiLSTM-CNN, so it would be PARENT_DIR's sibling
PROJECT_ROOT_GUESS = os.path.dirname(PARENT_DIR) # e.g. vidimu-tools/
GAITSEG_DIR = os.path.join(PARENT_DIR, "gaitseg")

# This global variable will be updated by hyperparameter_tuning script for each trial
OUTPUT_PLOTS_DIR = os.path.join(config.OUTPUT_DIR, "output_plots_default")
os.makedirs(OUTPUT_PLOTS_DIR, exist_ok=True) # Ensure default exists
...
try:
    ...
    print(f"Warning: Could not import gaitseg_utils from {GAITSEG_DIR}. Some plotting functions in utils.py might not work.")
    gal = None
...
def plot_training_history(history, fold_num=None, trial_num=None): # Added trial_num
    ...
    title_parts = []
    # Construct title prefix more carefully
    ...
    if title_prefix: title_prefix += " " # Add trailing space if not empty
    ...
    # Construct filename with trial and fold if present
    filename_parts = []
    if trial_num is not None: filename_parts.append(f"trial{trial_num}")
    if fold_num is not None: filename_parts.append(f"fold{fold_num}")
    filename_parts.append("training_history")
    plot_filename = "_".join(filename_parts) + ".png"

    # Use the global OUTPUT_PLOTS_DIR which is set by the calling script (train.py or hyp_tuning.py)
    # Ensure this global is correctly pointing to the trial-specific directory when called from Optuna
    global OUTPUT_PLOTS_DIR
    save_path = os.path.join(OUTPUT_PLOTS_DIR, plot_filename)
...
def plot_confusion_matrix_custom(y_true_flat, y_pred_flat, class_names, title='Confusion Matrix', fold_num=None, trial_num=None): # Added trial_num
    ...
    if not hasattr(y_true_flat, '__len__') or not hasattr(y_pred_flat, '__len__') or \
       len(y_true_flat) == 0 or len(y_pred_flat) == 0:
    ...
    # Filter y_pred_flat based on the original indices of y_true_flat that were valid
    ...
    if trial_num is not None: filename_parts.append(f"trial{trial_num}")
    if fold_num is not None: filename_parts.append(f"fold{fold_num}")
    filename_parts.append(title.replace(' ', '_').lower())
    filename_parts.append("confusion_matrix")
    plot_filename = "_".join(filename_parts) + ".png"
...
def load_checkpoint(filepath, model, optimizer=None, device=config.DEVICE):
    ...
    if not os.path.exists(filepath):
        ...
        return None # Return just None if file not found
    
    # Added weights_only=False for now as scaler object might be saved
    # Change to True if you only save model state_dict and basic Python types
...
def plot_model_predictions_vs_true_phases(timestamps, true_phases, predicted_phases, title_suffix="", trial_num=None):
    ...
    time_plot = np.array(timestamps)[:n] # Ensure numpy array for consistent slicing
    true_phases_plot = np.array(true_phases)[:n] if true_phases is not None and hasattr(true_phases, '__len__') and len(true_phases) > 0 else None
    predicted_phases_plot = np.array(predicted_phases)[:n]
    ...
    # Determine unique phases actually present for yticks
    present_phases = []
    if true_phases_plot is not None: present_phases.extend(np.unique(true_phases_plot))
    if predicted_phases_plot is not None: present_phases.extend(np.unique(predicted_phases_plot))
    
    ytick_values = sorted(list(set(present_phases)))
    if not ytick_values: # Fallback if no phases
        ytick_values = np.arange(config.NUM_CLASSES)

    ax.set_yticks(ytick_values) 
    ax.set_yticklabels([f"Phase {int(i)}" for i in ytick_values])
    ax.legend()
    ...
    filename_parts = []
    if trial_num is not None: filename_parts.append(f"trial{trial_num}")
    filename_parts.append(f"model_vs_true_phases{title_suffix.replace(' ', '_').lower()}")
    plot_filename = "_".join(filename_parts) + ".png"
    
    global OUTPUT_PLOTS_DIR
    save_path = os.path.join(OUTPUT_PLOTS_DIR, plot_filename)
    plt.savefig(save_path)
    print(f"Saved model vs true phases plot to {save_path}")
    plt.close(fig)
...
def plot_model_predictions_vs_angular_velocity(
    ...
    if gal is None:
        ...
        return
    if not hasattr(timestamps_csv, '__len__') or len(timestamps_csv) == 0:
    ...
    t_plot_master = np.array(timestamps_csv)
    predicted_phases_plot_src = np.array(predicted_phases_csv)
    ...
    t_plot_master_sub, omega_plot_aligned_sub, predicted_phases_csv_sub = t_plot_master[:n_plot], omega_plot_aligned[:n_plot], predicted_phases_plot_src[:n_plot]
    ...
    filename_parts = []
    if trial_num is not None: filename_parts.append(f"trial{trial_num}")
    filename_parts.append(f"model_phases_vs_ang_vel{title_suffix.replace(' ', '_').lower()}")
    plot_filename = "_".join(filename_parts) + ".png"

    global OUTPUT_PLOTS_DIR
    save_path = os.path.join(OUTPUT_PLOTS_DIR, plot_filename)
    plt.savefig(save_path)
    print(f"Saved model phases vs angular velocity plot to {save_path}")
    plt.close(fig)

# If __name__ == '__main__' block from your utils.py is fine for testing utils independently.