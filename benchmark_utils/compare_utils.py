...
def calculateAndPlotRMSE(inpath, inpath1, inpath2, subjects, activity, activity_legend, RMSE_SAMPLES=200, MAX_SYNC_OVERLAP=15, FINAL_LENGTH=None):
    ...
    ax[0].bar(x - 0.2, rmse_results["Maxine"], width=0.2, label="Maxine", align='center', color='#1F77B4')
    ax[0].bar(x, rmse_results["MotionBERT"], width=0.2, label="MMPose", align='center', color='#2CA02C')
    ax[0].bar(x + 0.2, rmse_results["MotionAGFormer"], width=0.2, label="MotionAGFormer", align='center', color=' #9467BD')
    ax[0].set_xticks(x)
    ...
    ax[1].bar(['Maxine', 'MotionBERT', 'MotionAGFormer'], 
               ...
               color=['#1F77B4', '#2CA02C', ' #9467BD'], width=0.6)
