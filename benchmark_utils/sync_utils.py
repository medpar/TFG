...
def plotFramesShiftToSyncrhonizeAllSubjectsOneActivity(csvlog,inpath,imu_inpath,outpath,subjects,activity,activity_legend,outputfilename=None,RMSE_SAMPLES=200,MAX_SYNC_OVERLAP=15,FINAL_LENGTH=None):
    ncols = 5
    nrows = len(subjects)
    ...
    for i,subject in enumerate(subjects):
        ...
        for trial in ["T01","T02","T03","T04","T05"]:
            ...
            inpathmotfull = os.path.join(imu_inpath,subject,motfilename)

            #print(f"READING: {inpathmotfull}")

            csv_folder = os.path.join(inpath,subject)
            imu_folder = os.path.join(imu_inpath,subject)
            imu_path = os.path.join(imu_folder, motfilename)
            csv_path = os.path.join(csv_folder, f"{motsubjacttrial}.csv")
            
            if not os.path.exists(imu_path) or not os.path.exists(csv_path):
                continue
            else:
                csv_folder = os.path.join(inpath,subject)
                imu_folder = os.path.join(imu_inpath,subject)
                # print("READING: "+inpathmotfull[:-4])
                dfmot, dfcsv = fileutil.readMOTandCSV(
        ...
        if remove_samples_imu>0:
            imufilenameRAW = os.path.join(imu_folder,subject+'_'+activity+'_'+trial+'.raw')
            addFramesShiftToCSVLog(csvlogfile,subject,activity,trial,imufilenameRAW,'raw',remove_samples_imu,rmse_centered,rmse_shift)
            
            imufilenameMOT = os.path.join(imu_folder,'ik_'+subject+'_'+activity+'_'+trial+'.mot')
        ...
        if remove_samples_video>0:
            videofilenameMP4 = os.path.join(csv_folder,subject+'_'+activity+'_'+trial+'.mp4')
            ...
            videofilenameCSV = os.path.join(csv_folder,subject+'_'+activity+'_'+trial+'.csv')
