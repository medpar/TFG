...
import numpy as np
import re
import benchmark_utils.signal_utils as sp
...
### .MOT UTILITIES (Updated)
def readMOTandCSV(csv_folder, imu_folder, subject, activity, trial):
    """Read IMU data from imu_folder and CSV data from csv_folder"""
    motprefixname = 'ik_'
    motfilename = motprefixname + subject + "_" + activity + "_" + trial + ".mot"
    
    # Read IMU data from IMU folder
    inpath_mot = os.path.join(imu_folder, motfilename)
    try:
    ...
    # Read CSV data from CSV folder
    csvfilename = subject + "_" + activity + "_" + trial + ".csv"
    inpath_csv = os.path.join(csv_folder, csvfilename)
    try:
        dfcsv = pd.read_csv(inpath_csv)
    except FileNotFoundError:    
        dfcsv = None
