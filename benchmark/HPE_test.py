...
def run_inference(in_base_path, out_base_path, selected_subjects=None, device='cuda:0'):
    ...
    for subject in selected_subjects:
        try:
            ...
            # Dictionary to store inferencer instances
            inferencers = {}
            ...
            for video_file in os.listdir(subject_in_path):
                if video_file.lower().endswith('.mp4'):
                    ...
                    try:
                        ...
                        inferencers[video_file] = MMPoseInferencer(
                            pose3d="motionbert_dstformer-ft-243frm_8xb32-120e_h36m",
                            device=device
                        )
                        # Run inference on the current video
                        result_generator = inferencers[video_file](
                            input_video,
                            show=False,
                            #return_vis=False,  # Para no sacar los videos
...
def main():
    ...
    if platform.system() == "Linux":
        dataset_path = '/mnt/b/VIDIMU'
        in_base_path = os.path.join(dataset_path, 'videosfullsize_videosoriginal/videosfullsize/videosoriginal')
        out_base_path = os.path.join(dataset_path, 'benchmark/pose3d_test')
        if not os.path.exists(out_base_path):
            os.makedirs(out_base_path)
        run_inference(in_base_path, out_base_path, selected_subjects, device='cuda:0')
